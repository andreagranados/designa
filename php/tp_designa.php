<?php
 class tp_designa extends toba_tp_basico
  ...
  protected $clase_encabezado = 'encabezado';	
	...
	/**
	 * Retorna el tï¿½tulo de la opreaciï¿½n actual, utilizado en la barra superior
	 */
	...
	protected function info_version()
	{
		...
		if( $version && ! (toba::proyecto()->get_id() == 'toba_editor') ) {
			...
			if($version_link) {
				$info .= "<hr /><a href=\'http://$version_link\' target=\"_bank\">Mï¿½s informaciï¿½n</a><br>";	
			}
			if($info) {
				$info = "Versiï¿½n: <strong>$version</strong><br>" . $info;
				$info = toba_recurso::ayuda(null, $info, 'enc-version');
			}else{
				$info = "class='enc-version'";
			}
			echo "<div $info >";		
			echo 'Versiï¿½n <strong>' . $version .'</strong>';
			echo '</div>';		
		}
	}	
