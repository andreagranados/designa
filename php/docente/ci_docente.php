...
class ci_docente extends toba_ci
{
	...
	function get_categ_estatuto($id){
        ...
        //--COSTO DE LA NUEVA DESIGNACION
            ...
            //busco las designaciones y reservas dentro del periodo vigente que son de la UA
            ...
            $sql="select  sum(case when d.desde<='".$pdia."' then 
                ...
                from 
                    ...
                    where not exists
                            (select * from novedad t_no 
                                where t_no.id_designacion=t_e.id_designacion and (t_no.tipo_nov=1 or t_no.tipo_nov=2)) 
                UNION
            ...
            $sql2="select (case when t_d.desde<='".$pdia."' then (case when t_d.hasta is null then 
                                            (((cast('".$udia."' as date)-cast('".$pdia."' as date))+1-(t_no.hasta-t_no.desde+1))*m_c.costo_diario*t_t.porc/100) 
                                            else (((t_d.hasta-'".$pdia."')+1-(t_no.hasta-t_no.desde+1))*m_c.costo_diario*t_t.porc/100) end )
             else (case when (t_d.hasta>='2016-01-31' or t_d.hasta is null) then ((('".$pdia."')-t_d.desde+1-(t_no.hasta-t_no.desde+1))*m_c.costo_diario*t_t.porc/100) else ((t_d.hasta-t_d.desde+1-(t_no.hasta-t_no.desde+1))*m_c.costo_diario*t_t.porc/100) end ) end )as costo 
            into temp auxi2
            ...
            $sql2 = toba::perfil_de_datos()->filtrar($sql2);//aplico el perfil de datos
        ...
        //obtengo inicio y fin del periodo
            $udia=$this->ultimo_dia_periodo();
            $pdia=$this->primer_dia_periodo();    
        //--COSTO DE ESTA DESIGNACION, busco lo que cuesta la categoria para el periodo actual
            $sql="select * from mocovi_costo_categoria m_c,mocovi_periodo_presupuestario m_e where m_c.id_periodo=m_e.id_periodo and m_e.actual and m_c.codigo_siu='".trim($cat)."'";
            $valor_categoria=toba::db('designa')->consultar($sql);
            //--dias trabajados
            $dias=0;
            ...
            $sql="select  sum"
             . "(case when d.desde<='".$pdia."' then (case when d.hasta is null then (((cast('".$udia."' as date)-cast('".$pdia."' as date))+1)*m_c.costo_diario*t_i.porc/100) else (((d.hasta-'".$pdia."')+1)*m_c.costo_diario*t_i.porc/100) end  ) else (case when (d.hasta>='".$udia."' or d.hasta is null) then ((('".$udia."')-d.desde+1)*m_c.costo_diario*t_i.porc/100) else ((d.hasta-d.desde+1)*m_c.costo_diario*t_i.porc/100) end  ) end )as costo 
                ...
                from (
                    (select * from designacion t_e 
                    where not exists(select * from novedad t_no 
                                    where t_no.id_designacion=t_e.id_designacion 
