...
class dt_designacionh extends toba_datos_tabla
{
    ...
    function get_tkd_historico($filtro=array()){
            ...
            $sql="select distinct c.*,case when c.id_reserva is null then t_do.apellido||', '||t_do.nombre else 'RESERVA: '||t_r.descripcion end as docente_nombre,t_do.legajo,t_i.porc,t_p.nombre as programa,t_dep.descripcion as id_departamento,t_a.descripcion as id_area,t_o.descripcion as id_orientacion
                from (
                    select distinct id_designacion,id_reserva,uni_acad,id_docente,desde,hasta,carac,cat_mapuche ,cat_estat,dedic,nro_540,id_departamento,id_area,id_orientacion,min(auditoria_fecha),'H' as hist
                    ...
                    group by id_designacion,id_reserva,uni_acad,id_docente,desde,hasta,carac,cat_mapuche ,cat_estat,dedic,nro_540,id_departamento,id_area,id_orientacion
                UNION
                    select distinct id_designacion,id_reserva,uni_acad,id_docente,desde,hasta,carac,cat_mapuche ,cat_estat,dedic,nro_540,id_departamento,id_area,id_orientacion,min(auditoria_fecha),'' as hist
                    from public_auditoria.logs_designacion a 
                    where a.nro_540=$nro and a.uni_acad='".$filtro['uni_acad']."'
