...
LEFT OUTER JOIN (select  t_m.id_materia,t_m.id_periodo,t_m.anio,count(distinct t_m.id_designacion) cant_desig_s 
    ...
    function get_listado_desig($des,$filtro){
        ...
        if (isset($filtro)) {
            ...
            if(isset($filtro['anio'])){
                $where.=" and t_a.anio=".$filtro['anio']['valor'];
            }     
        }
...
(select sub2.desc_materia,sub2.cod_carrera,sub2.rol,sub2.periodo,sub2.moddes,sub2.carga_horaria,string_agg(carreras_conj,'/') as carreras_conj 
 from 
 		(SELECT distinct t_a.id_designacion,t_d.uni_acad,t_a.id_materia,t_pe.desc_carrera||'('||cod_carrera||')' as cod_carrera,SUBSTRING(t_m.desc_materia,0,65)||'('||t_m.cod_siu||')' as desc_materia,substring(t_t.desc_item,0,5) as rol,t_a.id_periodo,t_p.descripcion as periodo,
