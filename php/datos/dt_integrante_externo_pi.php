...
class dt_integrante_externo_pi extends toba_datos_tabla
{
    ...
    function get_movi($id_p){
        $sql="select * from 
            (select t_do.apellido||', '||t_do.nombre as nombre,t_do.tipo_docum,t_do.nro_docum,funcion_p,carga_horaria,ua,t_i.desde,t_i.hasta,rescd 
            ...
            UNION
            select t_d.apellido||', '||t_d.nombre as nombre,t_d.tipo_docum,t_d.nro_docum,funcion_p,carga_horaria,institucion as ua,t_i.desde,t_i.hasta,rescd 
            from integrante_externo_pi t_i
            LEFT OUTER JOIN persona t_d ON (t_d.nro_docum=t_i.nro_docum and t_d.tipo_docum=t_i.tipo_docum)
            where t_i.pinvest=$id_p)a
            order by tipo_docum,nro_docum,nro_docum,desde;";
        return toba::db('designa')->consultar($sql);  
    }
