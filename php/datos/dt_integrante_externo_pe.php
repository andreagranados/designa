...
class dt_integrante_externo_pe extends designa_datos_tabla
{
    ...
    {
        $sql="select id_pext,apellido||', '||nombre as nombre,tipo_docum,nro_docum,fec_nacim,tipo_sexo,pais_nacim,funcion_p,carga_horaria from integrante_externo_pe where id_pext=".$id_p;
        return toba::db('designa')->consultar($sql);  
    }
    function get_plantilla($id_p){
        $sql="(select t_do.apellido||', '||t_do.nombre as nombre,t_do.tipo_docum,t_do.nro_docum,t_do.tipo_sexo,t_d.cat_estat||'-'||t_d.dedic as categoria,t_i.ua,t_i.carga_horaria,t_f.descripcion as funcion_p from  integrante_interno_pe t_i"
