     ...
     function get_listado_filtro($filtro=null)	{
            ...
            $sql="SELECT sub.*,m.anio as periodo,e.descripcion as estado
                  FROM 
                    ...
                    case when (p.id_estado='H' or p.id_estado='P') then sum(case when check_seac then case when opcion='D' then cant_seac*(hasta_seac-desde_seac+1)*ca.costo_diario else cant_seac*(hasta_seac-desde_seac+1)*(ca.costo_diario-ca2.costo_diario) end else 0 end) else 0 end as total_seac,
