...
/**
 * @description Operaciones sobre Areas
 */
class recurso_areas implements SIUToba\rest\lib\modelable //esta interface es documentativa, puede no estar
{
	...
	static function _get_modelos(){
		...
		$area = array_merge(
							array('idarea' => array('type' => 'integer',
												'_validar' => array(rest_validador::TIPO_INT))),
							$area_editar);
		return $models = array(
			'Area' => $area,
			'AreaEditar' => $area_editar

		);
	}

	protected function get_spec_area($con_imagen = true, $tipo= 'Area'){
	...
	$modelo = new modelo_area($idarea);
        ...
        if ($fila) {
                ...
                $fila = rest_hidratador::hidratar_fila($this->get_spec_area($incluir_imagen), $fila);
        }

	    /**Se escribe la respuesta*/
        rest::response()->get($fila);
    }
 // rest/areas/253/descripcion
    function get_descripcion_list ($idarea){
        $incluir_imagen = (bool) rest::request()->get('con_imagen', 0);
        $modelo = new modelo_area($idarea);
	...
	function get_list()
	{
		...
		$areas = modelo_area::get_areas($where, $order_by, $limit);
                

		/**Transformci�n al formato de la vista de la API
		 * Como buen ciudadano, se agrega un header para facilitar el paginado al cliente*/
		$areas = rest_hidratador::hidratar($this->get_spec_area(false), $areas);
		$cantidad = modelo_area::get_cant_areas($where);
		rest::response()->add_headers(array('Cantidad-Registros' => $cantidad));

		/**Se escribe la respuesta */
		rest::response()->get_list($areas);
	...
	function get_list__confoto()
	{
		...
		$areas = rest_hidratador::hidratar($this->get_spec_area(true), $areas);

		rest::response()->get($areas);
		rest::response()->add_headers(array('Cantidad-Registros' => $cantidad));
	}
	...
	function post_list()
	{
		...
		$nuevo = modelo_area::insert($datos_modelo);
     ...
     * @responses 404 No se pudo encontrar a la persona
     */
    function get_juegos_list($idarea)
    {
	    //se omite hidratador por simplicidad.
		$juegos = modelo_area::get_juegos($idarea);
	...
	protected function get_filtro_get_list()
	{
		...
		$filtro->agregar_campo("id_area", "area.idarea");
	...
	protected function procesar_input_edicion($relajar_ocultos = false)
	{
		...
		$spec_area = $this->get_spec_area(true, 'AreaEditar');
		rest_validador::validar($datos, $spec_area, $relajar_ocultos);
		...
		$datos = rest_hidratador::deshidratar_fila($datos, $spec_area);
		return $datos;
	}
