<?php
require_once("modelo/modelo_provincia.php");
 ...
 */
class recurso_provincias implements SIUToba\rest\lib\modelable //esta interface es documentativa, puede no estar
{
	...
	static function _get_modelos(){
		 ...
		 */
		$provincia_editar = array(
					'codigo_pcia' => array(	'type'     => 'integer', 
										'_validar' => array(rest_validador::OBLIGATORIO,
															rest_validador::TIPO_INT  )),
                                        'cod_pais' => array('type' => 'string'),
                                        'descripcion_pcia' => array(	'type' => 'string')
				);

		$provincia = array_merge(
							array('codigo_pcia' => array('type' => 'integer',
												'_validar' => array(rest_validador::TIPO_INT ))),
							$provincia_editar);
		return $models = array(
			'Provincia' => $provincia,
			'ProvinciaEditar' => $provincia_editar

		);
	}

	protected function get_spec_provincia($con_imagen = true, $tipo= 'Provincia'){
	...
	$modelo = new modelo_provincia($cod);
        ...
        if ($fila) {
                ...
                $fila = rest_hidratador::hidratar_fila($this->get_spec_provincia($incluir_imagen), $fila);
        }
	...
	function get_list()
	{
		...
		$provincia = modelo_provincia::get_provincias($where, $order_by, $limit);
		/**Transformci�n al formato de la vista de la API
		 * Como buen ciudadano, se agrega un header para facilitar el paginado al cliente*/
		$provincia = rest_hidratador::hidratar($this->get_spec_provincia(false), $provincia);
		$cantidad = modelo_provincia::get_cant_provincias($where);
		rest::response()->add_headers(array('Cantidad-Registros' => $cantidad));

		/**Se escribe la respuesta */
		rest::response()->get_list($provincia);
	...
	function get_list__confoto()
	{
		...
		$provincias = modelo_provincia::get_provincias($where, $order_by, $limit);
		$cantidad = modelo_provincia::get_cant_provincias($where);

		$provincias = rest_hidratador::hidratar($this->get_spec_provincia(true), $provincias);

		rest::response()->get($provincias);
		rest::response()->add_headers(array('Cantidad-Registros' => $cantidad));
	}
	...
	function post_list()
	{
		...
		$nuevo = modelo_provincia::insert($datos_modelo);
        ...
        /**
         * Se consume en GET /provincias/{id}/localidades
         ...
         */
        function get_localidades_list($cod)
        {
                //se omite hidratador por simplicidad.
                    $loc = modelo_provincia::get_localidades($cod);
                    rest::response()->get_list($loc);
        }
	...
	protected function get_filtro_get_list()
	{
		...
		$filtro->agregar_campo("nombre", "provincia.descripcion_pcia");
                $filtro->agregar_campo("codpais","provincia.cod_pais");

		$filtro->agregar_campo_ordenable("nombre", "provincia.descripcion_pcia");
		return $filtro;
	}
	...
	protected function procesar_input_edicion($relajar_ocultos = false)
	{
		...
		$spec_provincia = $this->get_spec_provincia(true, 'ProvinciaEditar');
		rest_validador::validar($datos, $spec_provincia, $relajar_ocultos);
		...
		$datos = rest_hidratador::deshidratar_fila($datos, $spec_provincia);
		return $datos;
	}
